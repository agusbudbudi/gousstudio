...
export default async function handler(req, res) {
  ...
  switch (action) {
    ...
    case 'create':
      ...
      try {
        const { data, error } = await supabase.from('clients').insert(req.body.payload).select().single();
        if (error) throw error;
    ...
    case 'update':
      ...
      try {
        ...
        const { error: clientError } = await supabase.from('clients').update(updates).eq('id', id);
        if (clientError) throw clientError;

        if (updates.full_name || updates.phone_number) {
          const cascade = {};
          if (updates.full_name) cascade.full_name = updates.full_name;
          if (updates.phone_number !== undefined) cascade.phone_number = updates.phone_number;
          await supabase.from('orders').update(cascade).eq('client_id', id);
        }
    ...
    case 'delete':
      if (req.method !== 'POST') return res.status(405).json({ message: 'Method not allowed' });
      try {
        const { error } = await supabase.from('clients').delete().eq('id', req.body.id);
        if (error) throw error;
        return res.status(200).json({ success: true });
      } catch (e) { return res.status(500).json({ message: e.message }); }
