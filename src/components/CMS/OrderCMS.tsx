...
const OrderCMS: React.FC = () => {
  ...
  const handleOrderChange = (field: string, value: any) => {
    ...
    if (field === "discount_value" || field === "discount_type") {
      ...
      if (type === "percentage" && parseFloat(String(val)) > 100) {
        newValue = 100;
      }

      if (
        type === "fixed" &&
        parseFloat(String(val)) > parseFloat(String(selectedOrder.price || 0))
      ) {
        newValue = parseFloat(String(selectedOrder.price || 0));
      }
    }

    let newOrder = { ...selectedOrder, [field]: newValue } as OrderItem;
