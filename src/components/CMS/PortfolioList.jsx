...
import { Edit3, Trash2, ExternalLink, ChevronUp, ChevronDown, ImageOff } from "lucide-react";
      ...
      {filteredItems.map((item, index) => {
        ...
        return (
          <motion.div
            ...
            className="group flex items-center gap-4 bg-white border border-slate-200 hover:border-brand-500/30 rounded-xl p-3 transition-all hover:shadow-sm"
          >
            {/* Reorder Controls - Only show if not searching */}
            {!isSearching && (
              <div className="flex flex-col items-center gap-1 min-w-[32px] border-r border-slate-50 pr-3">
                <button
                  disabled={index === 0}
                  ...
                  className={`p-1 rounded transition-colors ${
                    index === 0
                      ...
                      : "text-slate-400 hover:bg-brand-50 hover:text-brand-500 cursor-pointer"
                  }`}
            ...
            >
              ...
              <div className={`absolute inset-0 items-center justify-center text-slate-200 ${imageUrl ? 'hidden' : 'flex'}`}>
